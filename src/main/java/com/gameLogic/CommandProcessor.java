...
public class CommandProcessor {
    ...
    public void handleKeyInput(String keyPressed) {
        if (!commandState.equals(CommandState.NONE) && !keyPressed.equals("ENTER") || gameOver) {
            return;
        }
        ...
        switch (keyPressed) {
            case "LEFT", "RIGHT", "UP", "DOWN" -> {
                int delta = ((keyPressed.equals("LEFT") || keyPressed.equals("UP")) ? -1 : 1);
                char axis = ((keyPressed.equals("LEFT") || keyPressed.equals("RIGHT")) ? 'c' : 'r');
                updateMinimap.setVisibility(moveOnLevel(delta,axis));
    ...
    public int moveOnLevel(int movement, char dir){
        int[] playerCords = playerController.getCoords();
        return switch (dir) {
